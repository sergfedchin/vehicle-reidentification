...
class CustomDataSet4Veri776_withviewpont(Dataset):
    """Veri776 dataset maker."""

    def __init__(self, image_list_path: str, root_dir: str, is_train: bool = True, transform = None, viewpoints: str = None):
        """
        Args:
            image_list_path (str): Path to list of all filenames to include in the dataset.
            root_dir (str): Directory with all the images.
            transform (callable, optional): Optional transform to be applied
                on a sample.
        """
        with open(viewpoints, 'r') as f:
            self.viewpoints: dict = json.load(f)
        ...
        for line in lines:
            line = line.strip()
            view = self.viewpoints.get(line, None)
            if view is None:
                num_missing_views += 1
                continue
            self.view.append(int(view[-1]))
            self.names.append(line)
            self.labels.append(line.split('_')[0])
            self.cams.append(line.split('_')[1]) 
        if is_train:
        ...
        self.root_dir = root_dir
        self.transform = transform
        print(f"Missed viewpoint for {num_missing_views} out of {len(self.names)} images in {'train' if is_train else 'test'} dataset!")
    def get_class(self, idx):
        return self.labels[idx]
...
#     def __init__(self, lines, root_dir, is_train=True, mode=None, transform=None, test=False):
#         """
#         Args:
#             csv_file (string): Path to the csv file with annotations.
#             root_dir (string): Directory with all the images.
