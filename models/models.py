...
class MultiBranches(nn.Module):
    def __init__(self, branches, n_groups, pretrain_ongroups=True, end_bot_g=False, group_conv_mhsa=False, group_conv_mhsa_2=False, x2g = False, x4g=False):
        ...
        if n_groups > 0:
            ...
            for item in convlist:
                if item[1] == "downsample":
                    ...
                    m = model_ft[int(item[0])].get_submodule(item[1])
...
class FinalLayer(nn.Module):
    def __init__(self, class_num, branches, n_groups, losses="LBS", droprate=0, linear_num=False, return_f = True, circle_softmax=False, n_cams=0, n_views=0, LAI=False, x2g=False,x4g=False):
...
class MBR_model(nn.Module):         
    def __init__(self,
                 ...
                 branches: list[str],  # names of branches
                 n_groups: int,  # number of groups G to use in grouped convolutions
                 losses="LBS",
                 backbone="ibn",
                 droprate=0,
                 linear_num=False,
                 ...
                 end_bot_g=False,
                 group_conv_mhsa=False,
                 group_conv_mhsa_2=False,
                 x2g=False,
                 x4g=False,
                 LAI=False,
                 n_cams=0,
        ...
        self.modelL4 = MultiBranches(branches=branches,
                                     ...
                                     x2g=x2g,
                                     x4g=x4g)
        self.finalblock = FinalLayer(class_num=class_num,
                                     branches=branches,
                                     n_groups=n_groups,
                                     losses=losses,
                                     droprate=droprate,
