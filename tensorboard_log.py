...
class Logger(object):
    def __init__(self, data, logscalars=True, save_embed = True):
        run_id = 0
        savepath = os.path.join(os.path.curdir, 'logs', data['dataset'], data['model_arch'] + ('_LAI' if data["LAI"] else ''), str(run_id))
    ...
    def save_model(self, model):
        torch.save(model.state_dict(), os.path.join(self.savepath, 'last.pt'))

        if len(self.logscalars['Accuraccy/mAP']) > 1:
            if self.logscalars['Accuraccy/mAP'][-1] >= np.max(self.logscalars['Accuraccy/mAP'][:-1]):
                torch.save(model.state_dict(), os.path.join(self.savepath, 'best_mAP.pt'))

            if self.logscalars['Accuraccy/CMC1'][-1] >= np.max(self.logscalars['Accuraccy/CMC1'][:-1]):
                torch.save(model.state_dict(), os.path.join(self.savepath, 'best_CMC.pt'))
